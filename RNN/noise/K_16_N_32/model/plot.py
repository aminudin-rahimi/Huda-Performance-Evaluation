...
result = sio.loadmat('../../../../Results/Result-with-noise/LSTM_Result/K_16_N_32/lstm_result')
...
for i in range(len(train_ratio)):
    for j in range(len(train_snr)):
        for t in range(len(test_snr)):
            # Set epoch = 10**5
            NVE[i, j] += res_ber[i, j, t, -1] / map_ber[t]
        NVE[i, j] = NVE[i, j] / len(test_snr)

best_train_snr_index = np.zeros(len(train_ratio), dtype=int)
for i in range(len(train_ratio)):
...
plt.figure()
for i in range(len(train_ratio)):
    plt.semilogy(test_snr, res_ber[i, best_train_snr_index[i], :, -1].flatten(), line_style_1[i], lw=3)
...
plt.savefig('../../../../Results/Result-with-noise/LSTM_Result/K_16_N_32/ber.png')
# Show
plt.show()
