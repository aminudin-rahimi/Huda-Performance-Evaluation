    ...
    for tr_snr_index in range(len(train_snr)):
        ...
        for epoch in range(num_epoch):
            ...
            if (epoch + 1) % 1000 == 0:
                ...
                print(f'{datetime.now()}: epoch = {epoch + 1}, train_ratio = {train_ratio[ratio_index]}, train_snr = {train_snr[tr_snr_index]} dB ---> train_ber = {train_ber:.4f}, forward time = {duration:.3f} sec')
            ...
            if epoch + 1 in epoch_setting:
                ...
                for te_snr_index in range(len(test_snr)):
                    ...
                    print(f'{datetime.now()}: epoch = {epoch + 1}, train_ratio = {train_ratio[ratio_index]}, train_snr = {train_snr[tr_snr_index]} dB, test_snr = {test_snr[te_snr_index]} dB ---> ber = {test_ber:.4f}, forward time = {duration:.3f} sec')
...
print('\n*****************END***************')
print(f'For the backward time of MLP:\nbatch size = {batch_size}, total batch = {num_epoch}, time = {backward_batch_time_avg:.3f} sec/batch')
print(f'For the forward time of MLP:\ntest number = {len(x_test)}, total test = {forward_count}, time = {forward_time_avg:.3f} sec')
sio.savemat('../../../../Results/Result-with-noise/MLP_Result/K_4_N_8/mlp_result', {'ber_trainRatio_trainSNR_testSNR_epoch': res_ber, 'backward_batch_time_avg': backward_batch_time_avg, 'forward_time_avg': forward_time_avg})
